...
class Admincontato extends CI_Controller {
    private $title;
    private $class;
	private $model;
    ...
    function index($limit=1000, $offset=false) {	
        ...
        $config['total_rows'] = $this->{$this->model}->num_admincontatos($busca);
        ...
        $data->admincontatos = $this->{$this->model}->get_admincontatos($busca, $offset, $limit);
	...
	function excluir_contato($id=false) {
        $this->{$this->model}->delete_admincontato($id);
        redirect($this->class);
    }
