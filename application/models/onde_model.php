...
class Onde_model extends CI_Model{
    ...
    function get_adminblogs($busca = array()){
        $this->db->select('*');
        $this->db->from('entry4');
        if(isset($busca['entry4']) && $busca['entry4'] != ''){
            $this->db->like('entry_name',$busca['entry4']);
        }
